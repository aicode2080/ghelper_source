...
function error_html(err) {
  ...
  html +=
    '<h5><a href="error.html" target="_blank" class="btn btn-warning">' +
    getlang('error_button') +
    '</a></h5>';
  html += '</div></div>';
  $('body').html(html);
}
...
function check_proxy_config(show = 0) {
  chrome.proxy.settings.get({ incognito: false }, function (config) {
    if (config['levelOfControl'] != 'controlled_by_this_extension') {
      if (show) {
        $('#alert-messager').html(
          '<span class="label label-danger"><i class="glyphicon glyphicon-exclamation-sign"></i>' +
            getlang('check_proxy_config_title') +
            '</span>' +
            getlang('check_proxy_config_body')
        );
      } else {
        $('#alert-messager').html(
          '<span class="label label-danger"><i class="glyphicon glyphicon-exclamation-sign"></i>' +
